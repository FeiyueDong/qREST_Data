...
int qrest_from_bytes(qrest_c_byte_stream_t input, qrest_c_data_t *out_data) {
    ...
    try {
        // 1. 解析 FileHeader
        std::string header_bytes(reinterpret_cast<const char *>(input.bytes),
                                 16);
        ...
        std::memcpy(
            out_data->metadata_json.str, meta_bytes.c_str(), meta_bytes.size());
...
int qrest_to_bytes(qrest_c_string_t json_str,
    ...
    try {
        ...
        uint32_t data_point_count = static_cast<uint32_t>(meta.DataInfo.NPTS);
        ...
        // 2. 封装波形为 DataPacket
        std::span<const double> data_span(packet_data.data, packet_data.len);
